...
- Status  values such as Draft, InProgress, Pending, Tabled, ReadyForApproval, Approved, Rejected, ResearchDraft
- StatusTimestamp
- ApprovedTimestamp
- ApprovedBy
...
- set_ddr_status  sets the DDR status field for values such as Draft, InProgress, Pending, Tabled, ReadyForApproval, Approved, Rejected, ResearchDraft
...
### 4.7 User approval requests (client-side)

This group defines client-side operations that present explicit approval prompts to the user before the LLM performs server-side DDR changes.  
These operations are executed by the hosting client (for example, IDE extension, web UI, or agent shell) and do not call the DDR persistence layer directly.

- request_user_approval  asks the client to present an approval prompt and return a normalized decision

#### 4.7.1 request_user_approval

The LLM uses this operation when it has a concrete proposed change (such as approving a goal, chapter, or entire DDR) and must obtain explicit user consent before calling a server-side TUL-005 operation.

Request:

```json
{
  "operation": "request_user_approval",
  "payload": {
    "prompt": "string describing what the LLM wants the user to approve",
    "context": {
      "ddr_identifier": "optional string",
      "chapter_id": "optional string",
      "action": "optional string describing what will occur if approved"
    }
  }
}
```

- prompt  human readable text explaining exactly what the user is being asked to approve
- context  optional metadata to help the client and LLM correlate this approval to a DDR, chapter, or follow up operation
...
```json
{
  "ok": true,
  "result": {
    "decision": "Approved | Tabled | NotApproved",
    "timestamp": "client local timestamp",
...
- Approved  the user explicitly authorized the proposed action and the LLM may proceed with the corresponding server-side operation
- Tabled  the user wants to defer the action; the LLM must not proceed and may re-prompt at a later time
- NotApproved  the user rejected the proposal; the LLM must not proceed and should revise or abandon the proposed change
...
```json
{
  ...
  "error": {
    ...
    "message": "Client could not obtain user approval."
  }
}
```
...
- Deciding which server-side TUL-005 operation to call after an Approved decision
- Respecting Tabled and NotApproved decisions by not performing the proposed change
- Using the context payload to maintain a clear link between the approval request and the subsequent DDR operation
...
- A client-side approval mechanism that enables the LLM to obtain explicit user decisions before mutating DDR state

Implementation details such as the C sharp tool class, dependency injection wiring, client implementation of request_user_approval, and HTTP hosting are deferred to a follow up implementation DDR.
