...
namespace LagoVista.AI.Services.Pipeline
{
    public class AgentContextLoaderPipelineStap : PipelineStep, IAgentContextLoaderPipelineStap
    {
        ...
        protected override async Task<InvokeResult<AgentPipelineContext>> ExecuteStepAsync(AgentPipelineContext ctx)
        {
            ...
            {
                return InvokeResult<AgentPipelineContext>.FromError("Conversation Context specified in session not found in Agent Context.", "AGENT_CTX_LOADER_CONVERSATION_CONTEXT_NOT_FOUND_IN_AGENT_CONTEXT");
            }
