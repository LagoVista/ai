...
namespace LagoVista.AI.Services.Tools
{
    ...
    public class ListDdrsAgentTool : DdrAgentToolBase
    {
        ...
        public ListDdrsAgentTool(IDdrManager ddrManager, IAdminLogger adminLogger) : base(ddrManager, adminLogger)
        {
        }

        public const string ToolUsageMetadata = "Lists all DDRs in the system along with basic metadata. Used for navigation, search, and overview operations.";
        ...
        {
            return ToolSchema.Function(ToolName, @"List all DDRs for the current organization, returning identifier, title, status, and status timestamp on the first line.  '
                                ...
                                On the second row for each DDR you should return the summary if it's available.  If not lust leave the line off.", p =>
            {
            });
        }
        ...
        protected override async Task<InvokeResult<string>> ExecuteCoreAsync(JObject payload, AgentToolExecutionContext context, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var items = new JArray(response.Model.Select(m => new JObject { ["identifier"] = m.DdrIdentifier, ["title"] = m.Name, ["summary"] = String.IsNullOrEmpty(m.Summary) ? m.Description : m.Summary, ["status"] = m.Status, ["status_timestamp"] = m.StatusTimestamp }));
