...
";
        ...
        public static OpenAiToolDefinition GetSchema()
        {
            return ToolSchema.Function(ToolName, "Imports a DDR from a Markdown document (create-only). The DDR Markdown is authoritative. " + "The tool supports an LLM-assisted ingestion flow where identity may be extracted from Markdown and " + "derived fields are generated conditionally based on DDR type. The tool validates output and can " + "return parsed/generated values for human confirmation (dry-run) before persisting on confirmation.", p =>
            {
                ...
                p.String("status", "DDR status extracted from the Markdown (e.g., 'Approved'). If missing or ambiguous, set to null and set needsHumanConfirmation=true.");
                ...
                p.String("humanSummary", "Human-facing summary (1–2 full sentences) describing purpose and scope. Must not include procedural steps and must not introduce new rules. Required for all DDR types.");
                p.String("condensedDdrContent", "Condensed DDR content suitable as the default LLM reasoning substrate after identification. Must preserve all normative meaning; may omit examples/rationale/history; must not introduce new rules or interpretations. Required for all DDR types.");
                p.String("ragIndexCard", "RAG routing-only index card (1–2 sentences). Must include DDR ID, DDR Type, Status, Approval metadata, and a concise purpose statement. Must not contain normative rules or normative keywords (MUST, MUST NOT, SHOULD, MAY). Required for all DDR types.");
