...
namespace LagoVista.AI.Services.Tools
{
    ...
    public class GetDdrAgentTool : DdrAgentToolBase
    {
        ...
        public GetDdrAgentTool(IDdrManager ddrManager, IAdminLogger adminLogger) : base(ddrManager, adminLogger)
        {
        }

        public const string ToolUsageMetadata = @"Retrieves the full DDR, including metadata, chapters, and approval state. Used for review, summarization, or downstream analysis.  
      In other cases the ddr will be provided in content";
        public override string Name => ToolName;

        public const string ToolSummary = "get a ddr";
        ...
        {
            return ToolSchema.Function(ToolName, "Get a full DDR snapshot including metadata, goal, approval info, and all chapters.", p =>
            {
        ...
        protected override async Task<InvokeResult<string>> ExecuteCoreAsync(JObject payload, AgentToolExecutionContext context, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var chapterArray = new JArray(chapters.Select(c => new JObject { ["id"] = c.Id, ["title"] = c.Title, ["summary"] = c.Summary, ["details"] = c.Details, ["approved_by"] = c.ApprovedBy == null ? null : new JObject { ["id"] = c.ApprovedBy.Id, ["text"] = c.ApprovedBy.Text }, ["approved_timestamp"] = c.ApprovedTimestamp }));
                var resultBody = new JObject
                {
                    ...
                    ["goal"] = ddr.Goal,
                    ["goal_approved_by"] = ddr.GoalApprovedBy == null ? null : new JObject
                    {
                        ["id"] = ddr.GoalApprovedBy.Id,
                        ["text"] = ddr.GoalApprovedBy.Text
                    },
                    ["goal_approved_timestamp"] = ddr.GoalApprovedTimestamp,
                    ["approved_by"] = ddr.ApprovedBy == null ? null : new JObject
                    {
