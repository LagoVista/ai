...
namespace LagoVista.AI.Services.Tools
{
    ...
    public sealed class MatchWorkflowTool : IAgentTool
    {
        ...
        public async Task<InvokeResult<string>> ExecuteAsync(string argumentsJson, AgentToolExecutionContext context, CancellationToken cancellationToken = default)
        {
            ...
            {
                return InvokeResult<string>.FromError("MatchWorkflowTool requires a non-empty arguments object with 'userMessage'.");
            }
            ...
            {
                return InvokeResult<string>.FromError("MatchWorkflowTool requires 'userMessage'.");
            }
            ...
            try
            {
                ...
                var matches = defs.Model.Where(wf => wf.Status != WorkflowStatus.Disabled).Select(wf =>
                {
                ...
                }).Where(m => m != null).OrderByDescending(m => m.MatchScore).ToList();
        ...
        {
            return ToolSchema.Function(ToolName, "Match a free-form user message to one or more candidate workflows based on their intent patterns.", p =>
            {
                p.String("userMessage", "The raw user message text you want to match to known workflows.", required: true);
            });
        }
    }
}