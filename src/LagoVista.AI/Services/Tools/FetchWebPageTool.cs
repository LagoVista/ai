...
namespace LagoVista.AI.Services.Tools
{
    ...
    public sealed class FetchWebPageTool : IAgentTool
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(argumentsJson)) return InvokeResult<string>.FromError("agent_fetch_webpage requires a non-empty arguments object.");

            try
            {
                var args = JsonConvert.DeserializeObject<Args>(argumentsJson) ?? new Args();

                if (string.IsNullOrWhiteSpace(args.Url)) return InvokeResult<string>.FromError("agent_fetch_webpage requires 'url'.");
                if (!Uri.TryCreate(args.Url.Trim(), UriKind.Absolute, out var uri)) return InvokeResult<string>.FromError("agent_fetch_webpage requires a valid absolute URL.");
                if (uri.Scheme != Uri.UriSchemeHttp && uri.Scheme != Uri.UriSchemeHttps) return InvokeResult<string>.FromError("agent_fetch_webpage supports only http/https URLs.");
        ...
        private static string DecodeToString(byte[] bytes, string charset)
        {
            ...
            try
            {
                if (!string.IsNullOrWhiteSpace(charset)) return Encoding.GetEncoding(charset).GetString(bytes);
            }
            catch { }
        ...
        private static string ExtractText(string raw, string contentType)
        {
            ...
            if (!isHtml) return NormalizeWhitespace(raw);
        ...
        public static object GetSchema()
        {
            return new
            {
                type = "function",
                name = ToolName,
                description = "Fetch a public web page and return extracted text for analysis. Blocks private/local destinations and enforces size limits.",
                parameters = new
                {
                    type = "object",
                    properties = new
                    {
                        url = new { type = "string", description = "Absolute http/https URL to fetch." },
                        timeoutMs = new { type = "integer", description = "Request timeout in milliseconds (500–30000)." },
                        maxBytes = new { type = "integer", description = "Maximum bytes to download (4096–2000000)." },
                        maxChars = new { type = "integer", description = "Maximum characters returned after extraction (512–50000)." },
                        followRedirects = new { type = "boolean", description = "Follow redirects (default true)." }
                    },
                    required = new[] { "url" }
                }
            };
