...
";
        ...
        public async Task<InvokeResult<string>> ExecuteAsync(string argumentsJson, AgentToolExecutionContext context, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                if (op == "list")
                {
                    ...
                    return InvokeResult<string>.Create(JsonConvert.SerializeObject(new Result
                    {
                        Operation = "list",
                        Items = items,
                ...
                if (op == "upsert")
                {
                    ...
                    if (entry == null)
                    {
                        ...
                        var newId = string.IsNullOrWhiteSpace(args.Entry.KfrId)
                            ? $"KFR-{kind.ToString().ToUpperInvariant()}-{nextSeq:000}"
                            : args.Entry.KfrId.Trim();
                ...
                else  if (op == "evict")
                {
                    ...
                    if (blocked.Count > 0 && !force)
                    {
                        ...
                        return InvokeResult<string>.FromError(
                            $"session_kfr evict refused: the following KFR entries require resolution: {blockedIds}. " +
                            "Set force=true to dismiss/evict anyway.");
                    }
        ...
        public static object GetSchema()
        {
            return new
            {
                type = "function",
                name = ToolName,
                description = "Manage the Known Facts Registry (KFR) for the current session.",
                parameters = new
                {
                    type = "object",
                    properties = new
                    {
                        ...
                        {
                            type = "string",
                            description = "Operation to perform: list | clear | upsert | evict."
                        },
                        entry = new
                        {
                            type = "object",
                            properties = new
                            {
                                kfrId = new { type = "string", description = "Optional KFR identifier (auto-generated if omitted for upsert)." },
                                kind = new { type = "string", description = "KFR kind: Goal | Plan | ActiveContract | Constraint | OpenQuestion." },
                                value = new { type = "string", description = "Atomic KFR value (required for upsert)." },
                                requiresResolution = new { type = "boolean", description = "Whether this entry requires resolution before eviction/forgetting." }
                            }
                        },
                        kfrIds = new
                        {
                            ...
                            description = "KFR ids to evict (used with operation=evict)."
                        },
                        force = new
                        {
                            type = "boolean",
                            description = "If true, allow evicting entries that require resolution (used with operation=evict)."
                        }
                    },
