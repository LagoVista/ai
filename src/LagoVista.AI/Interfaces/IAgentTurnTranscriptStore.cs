...
namespace LagoVista.AI.Interfaces
{
    ...
    public interface IAgentTurnTranscriptStore
    {
        ...
        /// </summary>
        Task<InvokeResult<System.Uri>> SaveTurnRequestAsync(string orgId, string sessionId, string turnId, string requestJson, CancellationToken cancellationToken = default);

        /// <summary>
        /// Persist the full logical response payload for a turn and return
        /// an opaque blob URL or identifier that can be stored on the turn.
        /// </summary>
        Task<InvokeResult<System.Uri>> SaveTurnResponseAsync(string orgId, string sessionId, string turnId, string responseJson, CancellationToken cancellationToken = default);

        /// <summary>
        /// Load the previously stored logical request payload for a turn.
        /// Returns null if no transcript was stored.
        /// </summary>
        Task<InvokeResult<string>> LoadTurnRequestAsync(string orgId, string sessionId, string turnId, CancellationToken cancellationToken = default);

        /// <summary>
        /// Load the previously stored logical response payload for a turn.
        /// Returns null if no transcript was stored.
        /// </summary>
        Task<InvokeResult<string>> LoadTurnResponseAsync(string orgId, string sessionId, string turnId, CancellationToken cancellationToken = default);
    }
}
